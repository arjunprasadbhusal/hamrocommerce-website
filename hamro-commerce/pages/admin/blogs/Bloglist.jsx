...
import { Pencil, Trash2, Plus, FileText, Image as ImageIcon, AlertTriangle, X } from 'lucide-react'
...
export default function Bloglist() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      showAlert({ type: 'error', title: 'Error', message: err.message || 'Failed to load blogs' })
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(BLOG_ENDPOINTS.DELETE(deleteConfirm.id), {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
      ...
      if (data.success) {
        showAlert({ type: 'success', title: 'Success', message: 'Blog deleted successfully!' })
      ...
      showAlert({ type: 'error', title: 'Error', message: err.message || 'Failed to delete blog' })
      console.error(err)
    } finally {
      setDeleting(null)
    }
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {blogs.map((blog) => (
                        <tr key={blog.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-4">
                            <span className="text-sm font-semibold text-gray-800">{blog.title}</span>
                          </td>
                          <td className="px-4 py-4">
                          ...
                          <td className="px-4 py-4">
                            <div className="flex gap-2 justify-center">
                              ...
                              <button
                                onClick={() => handleDeleteRequest(blog.id, blog.title)}
